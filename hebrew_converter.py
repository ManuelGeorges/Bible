...
def convert_hebrew_xml_to_json(xml_path, output_path):
    ...
    try:
        ...
        print(f"Success! Hebrew Bible converted and saved to: {output_path}")

    except Exception as e:
        print(f"An error occurred during conversion: {e}")

if __name__ == "__main__":
    # Base path logic to handle execution from root
    xml_input = os.path.join("public", "data", "translations", "HebrewLeningradCodexBible.xml")
    json_output = os.path.join("public", "data", "translations", "Hebrew", "he.json")

    convert_hebrew_xml_to_json(xml_input, json_output)
