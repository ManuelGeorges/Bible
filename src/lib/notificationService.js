...
export const syncNotifications = async () => {
  ...
  try {
      ...
      if (settings.dailyVerse) {
        ...
        if (vDate > now) {
          notifications.push({
            ...
            smallIcon: ICON_NAME, iconColor: ICON_COLOR,
            extra: { url: "/" }
          });
        }
      }
    ...
    allPlans.forEach((plan, index) => {
      ...
      if (completedDays > 0 && completedDays < totalDays && settings.studyPlans) {
        ...
        notifications.push({
          id: 3000 + index,
          title: "متابعة الخطة 📖",
          body: `باقي لك ${remaining} يوم لتختم "${plan.title}". لا تتوقف الآن!`,
    ...
    const tips = [
      ...
      { title: "الخطط الدراسية الذكية", body: "هل جربت الاستعانة بمساعد أجيوس الذكي لوضع خطة دراسية تناسبك؟", url: "/studyPlans" },
      { title: "الخرائط التفاعلية", body: "تأكد من استكشاف الخرائط التفاعلية لدراسة اماكن الكتاب المقدس باسلوب جديد وعصري", url: "/maps" },
       { title: "البحث الذكي", body: "يمكنك الآن تجربة البحث باستخدام المشتقات ومرادفات الكلمة بمساعدة مساعد آجيوس الذكي!", url: "/search" },
       { title: "نظام المكافآت", body: "يوجد بالتطبيق نظام نقاط وأوسمة متقدم, يمكنك رؤية جميع انجازاتك عن طريق الذهاب لقسم النقاط", url: "/points" },
    ...
    if (appInfo?.updateAvailability === 2) {
      notifications.push({
        id: 6000,
